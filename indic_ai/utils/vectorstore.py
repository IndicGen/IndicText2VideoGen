...
# Initialize ChromaDB
    ...
    def add_text(self, case_id: str, text: str):
        ...
        # Fetch existing documents with the same case_id
        existing_results = self.collection.get(where={"case_id": case_id})

        # Extract existing chunks for comparison
        existing_chunks = set(existing_results["documents"]) if "documents" in existing_results else set()

        for chunk, embedding in zip(text_chunks, embeddings):
            if chunk in existing_chunks:
                continue  # Skip if the chunk already exists

            doc_id = str(uuid.uuid4())  # Unique ID for each new chunk
            self.collection.add(
                ids=[doc_id],
